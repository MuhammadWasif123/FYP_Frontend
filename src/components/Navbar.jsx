      ...
      <div className="flex items-center justify-between max-w-screen-xl px-4 py-2 mx-auto">
        ...
        >
          <ul className="flex mt-4 font-medium md:flex md:flex-row lg:space-x-8 lg:mt-0">
            <li>
              <Link to="/" className="block py-2 pl-3 pr-4 text-[#D9CAC2] rounded lg:p-0">Home</Link>
            </li>
            <li>
                 {/* 👇 Only show if logged in */}
              {user && (
                <Link to="/report-crime" className="block py-2 pl-3 pr-4 text-[#D9CAC2] rounded lg:p-0">
                  Report Crime
                </Link>
              )}
            </li>
        ...
        <div className="items-center justify-between lg:flex lg:w-auto lg:order-2 gap-2">
          {user ? (
            <>
              ...
              <img src={user.profile_image} alt="" className="rounded-full w-full" />
              </div>
              <button
                onClick={handleLogout}
                className="bg-[#e8d6d6] px-3 py-1 rounded"
              >
                Logout
              </button>
            </>
